    ...
    <div className="min-h-screen arabesque pb-40">
      ...
      <header className="sticky top-0 z-40 border-b border-border bg-background/80 backdrop-blur-xl">
        <nav className="mx-auto flex max-w-6xl items-center justify-between px-4 py-4 sm:px-6">
          <a href="#top" className="flex items-center gap-2">
            ...
            <span className="font-arabic text-xl text-gold-gradient">نور القرآن</span>
          </a>
      ...
      <section id="top" className="relative overflow-hidden">
        ...
        <div className="relative mx-auto max-w-4xl px-4 py-24 text-center sm:px-6 sm:py-32">
          <p className="animate-float font-arabic text-3xl text-primary sm:text-4xl">
            بِسْمِ اللَّهِ الرَّحْمَٰنِ الرَّحِيمِ
          </p>
          <h1 className="mt-8 text-4xl font-semibold leading-tight tracking-tight sm:text-6xl">
          ...
          <dl className="mx-auto mt-16 grid max-w-2xl grid-cols-3 gap-4">
            {[
              ...
              <div key={s.v} className="surface-card rounded-2xl px-3 py-5">
                <dt className="font-arabic text-2xl text-primary sm:text-3xl">{s.k}</dt>
          ...
          {RECITERS.map((r) => {
            ...
            return (
              ...
              >
                ...
                <p className="mt-1 text-xs text-muted-foreground">{r.country}</p>
                {active && (
      ...
      <section id="surahs" className="mx-auto max-w-6xl px-4 py-16 sm:px-6">
        ...
        <div className="relative mt-8">
          ...
          <input
            ...
            className="w-full rounded-full border border-border bg-card py-3.5 pl-11 pr-4 text-sm outline-none transition-shadow placeholder:text-muted-foreground focus:ring-2 focus:ring-ring"
          />
        </div>
        ...
        {!isLoading && filtered.length === 0 && (
          <p className="mt-10 text-center text-sm text-muted-foreground">Koi surah nahi mili.</p>
        )}
      </section>

      {/* Fazail */}
      <section id="fazail" className="mx-auto max-w-6xl px-4 py-16 sm:px-6">
        ...
        <div className="mt-8 grid gap-5 md:grid-cols-3">
          {VIRTUES.map((v) => (
            <article key={v.title} className="surface-card rounded-3xl p-7">
              <h3 className="font-urdu text-lg leading-loose text-primary">{v.title}</h3>
              <p className="mt-4 font-urdu text-sm leading-[2.4] text-muted-foreground">{v.text}</p>
              <p className="mt-5 text-xs tracking-wide text-primary/70">{v.ref}</p>
            </article>
          ))}
        </div>
      </section>
      ...
      {/* Duain */}
      <section id="duain" className="mx-auto max-w-6xl px-4 py-16 sm:px-6">
        ...
        <div className="mt-8 space-y-4">
          {DAILY_DUAS.map((d) => (
            <article key={d.ref} className="surface-card rounded-3xl p-7 text-center">
              <p className="font-arabic text-2xl leading-[2] text-primary sm:text-3xl">{d.arabic}</p>
              <p className="mx-auto mt-5 max-w-2xl font-urdu text-sm leading-[2.6] text-muted-foreground">
                {d.urdu}
              </p>
              <p className="mt-4 text-xs tracking-wide text-primary/70">{d.ref}</p>
            </article>
          ))}
        </div>
      </section>
      ...
      <footer className="mx-auto max-w-6xl border-t border-border px-4 py-12 text-center sm:px-6">
        <p className="font-arabic text-2xl text-gold-gradient">وَرَتِّلِ الْقُرْآنَ تَرْتِيلًا</p>
...
function SectionTitle({
  ...
  return (
    <div className="text-center">
      <span className="inline-flex items-center gap-2 rounded-full gold-border px-4 py-1.5 text-xs tracking-widest text-primary uppercase">
        {icon}
        {kicker}
      </span>
